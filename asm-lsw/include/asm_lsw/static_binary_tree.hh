...
namespace asm_lsw { namespace detail {
	...
	private:
		...
		typename t_tree::value_type dereference() const;
	};
}}
...
namespace asm_lsw
{
	...
	public:
		...
		typedef sdsl::int_vector <std::numeric_limits <t_value>::digits>		vector_type;
	...
	protected:
		...
		void fill_used_indices(t_int_vector const &values, size_type const target, size_type const begin, size_type const end);

		template <typename t_int_vector>
		void fill_values(t_int_vector const &values, size_type const target, size_type const begin, size_type const end);
		
		bool lower_bound_subtree(value_type const &key, size_type idx, const_iterator &it /* out */) const;
		
	public:
		static_binary_tree() {}
		
		template <typename t_int_vector>
		static_binary_tree(t_int_vector &values);
	...
	template <typename t_value>
	template <typename t_int_vector>
	void static_binary_tree <t_value>::fill_used_indices(t_int_vector const &values, size_type const target, size_type const lb, size_type const rb)
	{
		size_type const mid(lb + (rb - lb) / 2);
		m_used_indices[target] = 1;
		
		if (lb < mid)
			fill_used_indices(values, left_child(target), lb, mid - 1);
		
		if (mid < rb)
			fill_used_indices(values, right_child(target), 1 + mid, rb);
	}
	
	
	template <typename t_value>
	template <typename t_int_vector>
	void static_binary_tree <t_value>::fill_values(t_int_vector const &values, size_type const target, size_type const lb, size_type const rb)
	{
		size_type const mid(lb + (rb - lb) / 2);
		size_type pos(m_used_indices_r1_support.rank(target));
		m_values[pos] = values[mid];
		
		if (lb < mid)
			fill_values(values, left_child(target), lb, mid - 1);
		
		if (mid < rb)
			fill_values(values, right_child(target), 1 + mid, rb);
	}
	
	
	template <typename t_value>
	template <typename t_int_vector>
	static_binary_tree <t_value>::static_binary_tree(t_int_vector &values)
	{
		auto const size(values.size());
		if (size)
		{
			{
				vector_type vec(size, 0);
			...
			fill_used_indices(values, 0, 0, size - 1);
	...
	template <typename t_value>
	bool static_binary_tree <t_value>::left_child_c(size_type &i /* inout */) const
	{
	...
	template <typename t_value>
	bool static_binary_tree <t_value>::right_child_c(size_type &i /* inout */) const
	{
	...
	template <typename t_value>
	bool static_binary_tree <t_value>::parent_c(size_type &i /* inout */) const
	{
		if (i)
		{
			i = parent(i);
	...
	template <typename t_value>
	bool static_binary_tree <t_value>::find_leftmost(size_type &i /* inout */) const
	{
		bool found(false);
		while (true)
		{
	...
	template <typename t_value>
	auto static_binary_tree <t_value>::find(value_type const &key) const -> const_iterator
	{
		if (empty())
			return cend();
		...
		while (true)
		{
			size_type value_idx(m_used_indices_r1_support.rank(idx));
			if (key < m_values[value_idx])
			{
				if (!left_child_c(idx))
					return cend();
			}
			else if (m_values[value_idx] < key)
			{
	...
	template <typename t_value>
	auto static_binary_tree <t_value>::lower_bound(value_type const &key) const -> const_iterator
	{
		if (empty())
			return cend();
	...
	template <typename t_value>
	bool static_binary_tree <t_value>::lower_bound_subtree(value_type const &key, size_type idx, const_iterator &it /* out */) const
	{
	...
	template <typename t_value>
	void static_binary_tree <t_value>::load(std::istream& in)
	{
		m_values.load(in);
	...
	template <typename t_value>
	void static_binary_tree <t_value>::print() const
	{
...
namespace asm_lsw { namespace detail {
	...
	template <typename t_tree>
	auto static_binary_tree_iterator_tpl <t_tree>::dereference() const -> typename t_tree::value_type
	{
		assert(m_idx < m_tree->m_used_indices.size());
		size_type const value_idx(m_tree->m_used_indices_r1_support.rank(m_idx));
		auto const retval(m_tree->m_values[value_idx]);
		return retval;
	}
}}
